document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayCars(filteredCars) {
        ...
        filteredCars.forEach(car => {
            ...
            carItem.classList.add('car-item', 'bg-white', 'dark:bg-gray-800', 'rounded', 'shadow', 'p-4', 'flex', 'flex-col');

            carItem.innerHTML = `
                <img src="${car.image}" alt="${car.name}" class="w-full h-48 object-cover rounded mb-4" />
                <div class="car-details flex flex-col gap-1">
                    <h3 class="text-xl font-semibold">${car.name} (${car.brand})</h3>
                    <p>${car.description}</p>
                    <p>Tipo: ${car.type.charAt(0).toUpperCase() + car.type.slice(1)}</p>
                    <span class="price font-bold text-green-600">$${car.price.toLocaleString()}</span>
                    <a href="detalle.html?id=${car.id}" class="mt-3 px-3 py-1 bg-green-600 text-white rounded hover:bg-green-700 transition text-center">Ver Detalles</a>
                </div>
            `;
